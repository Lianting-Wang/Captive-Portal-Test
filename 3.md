...
1. **Function Parameters:**
   ...
   - **server:** The DNS server IP to forward the query to. Default is Google's DNS server ('8.8.8.8').
...
2. **Exclude Certain IPs:**
   ...
   - **Code Example:** 
     ```python
     if packet[IP].src != '10.0.0.1' and packet[IP].src != '8.8.8.8':
     ```
   - **Explanation:** Replace `'10.0.0.1'` with the interceptor's IP and `'8.8.8.8'` with the DNS server's IP. This ensures that packets from/to these IPs are not handled by the interceptor.
